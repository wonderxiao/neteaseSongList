...
def getRawMusic(id,folder_name,name):
    ...
    index_temp = name.find('（')
    if index_temp >= 0:
        name = name[:index_temp]
    #fpath = os.path.join(folder_name, '%s.mp3' % name)
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    fpath_temp = os.path.join(folder_name, '%s.mp3' % name)
    if  not os.path.exists(fpath_temp):
        r = requests.get(url, headers=headers).content
        f = open(fpath_temp,'wb')
        f.write(r)
        f.close()
    print('Download Finish.')


def getAllSongsByListId(id):
...
def search_song_by_name(name):
    ...
    r = requests.get(url=search_url,params=params,headers=headers).content
    resp_js = json.loads(r)
    if resp_js['code'] == 200 and resp_js['result']['songCount'] > 0:
        ...
        resp = requests.get(detail_url,headers=headers).content
        song_js = json.loads(resp)
        return song_js['songs'][0]
    else:
        return None
if __name__ == '__main__':
    #id = input('Please input songList id:')
    #getAllSongsByListId(id)
    while True:
        name = input('Please input song name:')
        songInfo = search_song_by_name(name)
        getRawMusic(songInfo['id'],'addSong',name)
#569213220